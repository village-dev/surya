...
def get_regions_from_detection_result(
    ...
    for bbox in vertical_line_bboxes:
        ...
        logits[:, vert_bbox[1] : vert_bbox[3], vert_bbox[0] : vert_bbox[2]] = (
    ...
    detected_boxes = sorted(
        detected_boxes, key=lambda x: x.confidence or 0, reverse=True
    )
    # Expand bbox to cover intersecting lines
    box_lines: defaultdict[int, list[list[float]]] = defaultdict(list)
...
def get_regions(
    ...
    for i in range(1, len(id2label)):  # Skip the blank class
        ...
        bbox = get_and_clean_boxes(
            heatmap, heatmap_p90s[i], list(reversed(heatmap.shape)), orig_size
        )
        for bb in bbox:
