...
from surya.model.ordering.config import (
    MBartOrderConfig,
...
def load_model(
    checkpoint=settings.ORDER_MODEL_CHECKPOINT,
    ...
    model = model.eval()
    print(
        f"Loaded reading order model {checkpoint} on device {device} with dtype {dtype}"
    )
    return model
