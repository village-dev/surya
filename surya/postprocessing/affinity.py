...
def get_detected_lines(
    image: torch.Tensor, slope_tol_deg=2, vertical=False, horizontal=False
) -> List[ColumnLine]:
    assert not (vertical and horizontal)
    new_image = image.to(torch.float32) * 255  # Convert to 0-255 range
    if vertical or horizontal:
        new_image = get_detected_lines_sobel(new_image.numpy(), vertical)
