...
def batch_table_recognition(
    ...
    for i in tqdm(range(0, len(images), batch_size), desc="Recognizing tables"):
        ...
        batch_table_cells = [
            sort_bboxes(page_bboxes) for page_bboxes in batch_table_cells
        ]  # Sort bboxes before passing in
        batch_list_bboxes = [
            [block["bbox"] for block in page] for page in batch_table_cells
        ]
        ...
        batch_decoder_input = [
            [[model.config.decoder.bos_token_id] * 5] for _ in range(current_batch_size)
        ]
        ...
        max_tokens = min(
            batch_bbox_counts[:, 1].max().item(), settings.TABLE_REC_MAX_BOXES
        )
        ...
        with torch.inference_mode():
            ...
            while token_count < max_tokens:
                ...
                done = (rowcol_preds == processor.tokenizer.eos_id) | (
                    rowcol_preds == processor.tokenizer.pad_id
                )
                done = done
                ...
                batch_decoder_input = torch.cat(
                    [box_preds.unsqueeze(1), rowcol_preds.unsqueeze(1).unsqueeze(1)],
                    dim=-1,
                )
        ...
        for j, (preds, input_cells, orig_size) in enumerate(
            zip(batch_predictions, batch_table_cells, orig_sizes)
        ):
            ...
            for row_idx, row in enumerate(bb_rows):
                rows.append(TableRow(bbox=row, row_id=row_idx))
