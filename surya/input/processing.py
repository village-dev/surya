...
def get_total_splits(image_size: Tuple[int, int], processor: SegformerImageProcessor):
...
def split_image(img: Image.Image, processor: SegformerImageProcessor):
...
def prepare_image_detection(
    img: Image.Image, processor: SegformerImageProcessor
) -> torch.Tensor:
    new_size = (processor.size["width"], processor.size["height"])

    # This double resize actually necessary for downstream accuracy
    img.thumbnail(new_size, Image.Resampling.BOX)
    img = img.resize(
        new_size, Image.Resampling.BOX
    )  # Stretch smaller dimension to fit new size

    # permute from HWC to CHW
    img_arr = torchvision.transforms.functional.pil_to_tensor(img)
    img_arr = processor([img_arr])["pixel_values"][0]
    return img_arr


def open_pdf(pdf_filepath: str):
    return pypdfium2.PdfDocument(pdf_filepath)


def get_page_images(
    doc: pypdfium2.PdfDocument, indices: List[int], dpi: int = settings.IMAGE_DPI
):
    renderer = doc.render(
