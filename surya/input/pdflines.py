...
def get_page_text_lines(
    filepath: str,
    page_idxs: list,
    out_sizes: list,
    flatten_pdf: bool = settings.FLATTEN_PDF,
) -> list:
    assert len(page_idxs) == len(out_sizes)
    pages_text = dictionary_output(
        filepath,
        sort=False,
        page_range=page_idxs,
        keep_chars=True,
    ...
    for full_text, out_size in zip(pages_text, out_sizes):
        ...
        for block in full_text["blocks"]:
            for line in block["lines"]:
                line["bbox"] = [
                    line["bbox"][0] * text_w_scale,
                    line["bbox"][1] * text_h_scale,
                    line["bbox"][2] * text_w_scale,
                    line["bbox"][3] * text_h_scale,
                ]
                for span in line["spans"]:
                    for char in span["chars"]:
                        char["bbox"] = [
                            char["bbox"][0] * text_w_scale,
                            char["bbox"][1] * text_h_scale,
                            char["bbox"][2] * text_w_scale,
                            char["bbox"][3] * text_h_scale,
                        ]
    return pages_text


def get_dynamic_gap_thresh(
    full_text: dict, img_size: list, default_thresh=0.01, min_chars=100
):
    space_dists = []
    for block in full_text["blocks"]:
        for line in block["lines"]:
            for span in line["spans"]:
                for i in range(1, len(span["chars"])):
                    ...
                    if full_text["rotation"] == 90:
                        space_dists.append(
                            (char2["bbox"][0] - char1["bbox"][2]) / img_size[0]
                        )
                    elif full_text["rotation"] == 180:
                        space_dists.append(
                            (char2["bbox"][1] - char1["bbox"][3]) / img_size[1]
                        )
                    elif full_text["rotation"] == 270:
                        space_dists.append(
                            (char1["bbox"][0] - char2["bbox"][2]) / img_size[0]
                        )
                    else:
                        space_dists.append(
                            (char1["bbox"][1] - char2["bbox"][3]) / img_size[1]
                        )
    cell_gap_thresh = (
        np.percentile(space_dists, 80)
        if len(space_dists) > min_chars
        else default_thresh
    )
...
def get_table_blocks(
    tables: list, full_text: dict, img_size: list, table_thresh=0.8, space_thresh=0.01
):
    # Returns coordinates relative to input table, not full image
    table_texts = []
    space_thresh = max(
        space_thresh,
        get_dynamic_gap_thresh(full_text, img_size, default_thresh=space_thresh),
    )
    for table in tables:
        table_poly = PolygonBox(
            polygon=[
                [table[0], table[1]],
                [table[2], table[1]],
                [table[2], table[3]],
            ...
            for line in block["lines"]:
                ...
                for span in line["spans"]:
                    for char in span["chars"]:
                        same_span = False
                        if curr_span:
                            same_span = is_same_span(
                                char, curr_box, img_size, space_thresh, rotation
                            )
                        ...
                        if curr_span is None:
                            ...
                            curr_span += char["char"]
                            curr_box = [
                                min(curr_box[0], char["bbox"][0]),
                                min(curr_box[1], char["bbox"][1]),
                                max(curr_box[2], char["bbox"][2]),
                                max(curr_box[3], char["bbox"][3]),
                            ]
                        else:
                            if curr_span.strip():
