...
def load_pdf(
    ...
    load_text_lines=False,
    flatten_pdf=settings.FLATTEN_PDF,
):
    ...
    if start_page:
        assert (
            start_page < last_page and start_page >= 0
        ), f"Start page must be between 0 and {last_page}"
    else:
        start_page = 0

    if max_pages:
        assert max_pages >= 0, "Max pages must be greater than 0"
    ...
    if load_text_lines:
        from surya.input.pdflines import (
            get_page_text_lines,
        )  # Putting import here because pypdfium2 causes warnings if its not the top import

        text_lines = get_page_text_lines(
            pdf_path, page_indices, [i.size for i in images], flatten_pdf=flatten_pdf
        )
    doc.close()
