...
def run_recognition(
    ...
    assert len(images) == len(
        langs
    ), "You need to pass in one list of languages for each image"
    ...
    rec_predictions, _ = batch_recognition(
    ...
    for idx, (image, lang) in enumerate(zip(images, langs)):
        ...
        for i in range(len(image_lines)):
            ...
            text_lines.append(TextLine(text=image_lines[i], polygon=poly))
...
def run_ocr(
    ...
    rec_predictions, confidence_scores = batch_recognition(
        all_slices, all_langs, rec_model, rec_processor, batch_size=batch_size
    )
