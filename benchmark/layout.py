...
def main():
    ...
    dataset = datasets.load_dataset(
        settings.LAYOUT_BENCH_DATASET_NAME, split=f"train[:{args.max}]"
    )
    ...
    label_alignment = {  # First is publaynet, second is surya
        ...
        "Text": [
            ["Text", "List"],
            ["Text", "Formula", "Footnote", "Caption", "List-item"],
        ],
    ...
    for idx, pred in enumerate(layout_predictions):
        ...
        for label_name in label_alignment:
            ...
            correct_bboxes = [
                b for b, l in zip(row["bboxes"], row["labels"]) if l in correct_cats
            ]
    ...
    for l in layout_types:
        for m in metric_types:
            ...
            for page in page_metrics:
                metric.append(
                    page_metrics[page][l][m] * page_metrics[page][l]["weight"]
                )
    ...
    for layout_type in layout_types:
        table_data.append(
            ...
            + [f"{mean_metrics[layout_type][m]:.2f}" for m in metric_types]
        )
