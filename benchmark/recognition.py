...
def main():
    ...
    parser.add_argument(
        "--max", type=int, help="Maximum number of pdf pages to OCR.", default=None
    )
    parser.add_argument(
        "--debug",
        type=int,
    ...
    parser.add_argument(
        "--tesseract",
        action="store_true",
        help="Run tesseract instead of surya.",
        default=False,
    )
    parser.add_argument(
        "--langs",
        type=str,
        help="Specify certain languages to benchmark.",
        default=None,
    )
    parser.add_argument(
    ...
    benchmark_stats = {
        "surya": {
            ...
            "lang_scores": {
                l: sum(scores) / max(1, len(scores))
                for l, scores in surya_scores.items()
            },
    ...
    if args.tesseract:
        ...
        tess_predictions = tesseract_ocr_parallel(
            tess_imgs, tess_bboxes, tess_langs, cpus=args.tess_cpus
        )
        ...
        benchmark_stats["tesseract"] = {
            ...
            "lang_scores": {
                l: sum(scores) / len(scores) for l, scores in tess_scores.items()
            },
    ...
    table_data = [
        ...
        + [benchmark_stats["surya"]["lang_scores"][l] for l in key_languages],
    ]
    if args.tesseract:
        table_data.append(
            [
            ...
            + [
                benchmark_stats["tesseract"]["lang_scores"].get(l, 0)
                for l in key_languages
            ]
        )
